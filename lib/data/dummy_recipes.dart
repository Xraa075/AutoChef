import 'package:autochef/models/recipe.dart';

List<Recipe> dummyRecipes = [
  Recipe(
    name: "Pancake",
    category: "Western",
    image: "lib/assets/images/pancake.jpg",
    time: 5,
    calories: 312,
    protein: 13,
    carbs: 9,
    difficulity: "Mudah",
    ingredients: [
      "100 gram Tepung Terigu",
      "2 sendok makan Gula Pasir",
      "1 butir telur",
      "1 sendok teh Baking Powder"
    ],
    steps: [
      "Campur bahan kering: tepung, gula, baking powder, dan garam.",
      "Kocok telur, tambahkan susu dan mentega yang dilelehkan.",
      "Gabungkan adonan, aduk hingga rata.",
      "Panaskan wajan dengan sedikit mentega.",
      "Tuang adonan, masak hingga muncul gelembung lalu balik.",
      "Angkat dan sajikan dengan topping favorit."
    ],
  ),
  Recipe(
    name: "Pancake Coklat",
    category: "Western",
    image: "lib/assets/images/pancake.jpg",
    time: 5,
    calories: 312,
    protein: 13,
    carbs: 9,
    difficulity: "Mudah",
    ingredients: [
      "100 gram Tepung Terigu",
      "2 sendok makan Gula Pasir",
      "1 butir telur",
      "1 sendok teh Baking Powder"
    ],
    steps: [
      "Campur bahan kering: tepung, gula, baking powder, dan garam.",
      "Kocok telur, tambahkan susu dan mentega yang dilelehkan.",
      "Gabungkan adonan, aduk hingga rata.",
      "Panaskan wajan dengan sedikit mentega.",
      "Tuang adonan, masak hingga muncul gelembung lalu balik.",
      "Angkat dan sajikan dengan topping favorit."
    ],
  ),
  Recipe(
    name: "Omelet Keju",
    category: "Western",
    image: "lib/assets/images/pancake.jpg",
    time: 10,
    calories: 210,
    protein: 15,
    carbs: 2,
    difficulity: "Mudah",
    ingredients: [
      "2 butir telur",
      "50 gram Keju Cheddar",
      "1 sendok makan Susu Cair",
      "1/2 sendok teh Garam"
    ],
    steps: [
      "Kocok telur bersama garam dan susu.",
      "Panaskan wajan dengan sedikit minyak.",
      "Tuang adonan telur, masak dengan api kecil.",
      "Taburkan keju di atasnya, lalu lipat omelet.",
      "Masak hingga matang, lalu angkat dan sajikan."
    ],
  ),
  Recipe(
    name: "French Toast",
    category: "Western",
    image: "lib/assets/images/pancake.jpg",
    time: 10,
    calories: 250,
    protein: 8,
    carbs: 30,
    difficulity: "Mudah",
    ingredients: [
      "2 lembar Roti Tawar",
      "1 butir Telur",
      "50 ml Susu Cair",
      "1 sendok makan Mentega"
    ],
    steps: [
      "Kocok telur dan susu hingga tercampur rata.",
      "Celupkan roti ke dalam campuran telur.",
      "Panaskan wajan dengan mentega, lalu goreng roti hingga kecoklatan.",
      "Balik dan masak sisi lainnya.",
      "Angkat dan sajikan dengan madu atau buah-buahan."
    ],
  ),
  Recipe(
    name: "Mie Goreng",
    category: "Asian",
    image: "lib/assets/images/pancake.jpg",
    time: 15,
    calories: 400,
    protein: 10,
    carbs: 50,
    difficulity: "Mudah",
    ingredients: [
      "200 gram Mie Telur",
      "2 sendok makan Kecap Manis",
      "1 butir Telur",
      "50 gram Ayam Suwir"
    ],
    steps: [
      "Rebus mie hingga matang, lalu tiriskan.",
      "Panaskan minyak, tumis bawang putih hingga harum.",
      "Masukkan telur, aduk hingga berbentuk orak-arik.",
      "Tambahkan ayam dan mie, lalu beri kecap dan bumbu lainnya.",
      "Aduk rata, masak hingga bumbu meresap, lalu sajikan."
    ],
  ),
  Recipe(
    name: "Salad Buah",
    category: "Healthy",
    image: "lib/assets/images/pancake.jpg",
    time: 10,
    calories: 150,
    protein: 2,
    carbs: 35,
    difficulity: "Mudah",
    ingredients: [
      "100 gram Stroberi",
      "100 gram Melon",
      "100 gram Anggur",
      "50 ml Yogurt"
    ],
    steps: [
      "Potong semua buah sesuai selera.",
      "Campurkan dalam satu mangkuk.",
      "Tuangkan yogurt di atasnya.",
      "Aduk rata, lalu sajikan dingin."
    ],
  ),
];
